...
if (empty($row_id) || empty($_POST['link']) || empty($_SESSION['user'])) {
	http_response_code(500);
}

$statement = $session->prepare('UPDATE data USING TTL 0 SET hell=false, shortlink=?, url=?, user=?, latitude=?, longitude=?, radius=? WHERE id=?;');
